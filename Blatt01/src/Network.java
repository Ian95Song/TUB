...
public class Network {
    ...
    public Network(int inter, int receptors, int cortical) {
        // TODO
        Neuron[] neurons = new Neuron[inter+receptors+cortical];

        int nblue = receptors/3;
        for(int i=0; i<nblue; i++){
            neurons[i] = new Photoreceptor(i,"blue");
        }
        for(int i=nblue; i<nblue*2; i++){
            neurons[i] = new Photoreceptor(i,"green");
        }
        for(int i=nblue*2; i<receptors; i++){
...
//        n1.addSynapse(synapse);
        if (n1 instanceof Photoreceptor){
            ...
            n1.outgoingsynapses = outgoingsynapses;
    ...
    public double[] signalprocessing(double[] input) {
        ...
        //devided by colorreceptor amount
        double[] count = this.countColorreceptors();
        for (int j=0; j<3; j++){
            signal[j] = signal[j]/count[j];
        }
